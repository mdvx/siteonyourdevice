...
void trayIconOnMenu(GtkStatusIcon *status_icon, guint button, guint activate_time,
                    gpointer user_data) {
...
class GtkMainWindow::EventHandler {
 public:
  explicit EventHandler(GtkMainWindow* parent)
      : parent_(parent) {
  }

  static void click_external_domain_change_callback(GtkToggleButton *button,
  ...
  static void click_browse_content_callback(GtkButton *button, gpointer callback_data) {
      ...
      GtkWidget * chose_content_path  = gtk_file_chooser_dialog_new("Select content folder",
                                                                    GTK_WINDOW(parent->window_),
                                                                    GTK_FILE_CHOOSER_ACTION_OPEN,
                                                                    GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
                                                                    GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT,
                                                                    NULL);
      gtk_file_chooser_set_action(GTK_FILE_CHOOSER(chose_content_path),
...
GtkMainWindow::GtkMainWindow(network::NetworkController *controller)
    : GuiNetworkEventHandler(controller), connect_button_(NULL),
      domain_text_(NULL), port_text_(NULL), login_text_(NULL), password_text_(NULL),
      content_path_text_(NULL), browse_content_button_(NULL), external_host_(NULL),
      external_port_(NULL) {

    evhandler_ = new EventHandler(this);

    window_ = gtk_window_new(GTK_WINDOW_TOPLEVEL);
    gtk_window_set_icon_from_file(GTK_WINDOW(window_),
                                  "/usr/share/icons/" PROJECT_NAME_LOWERCASE ".png", NULL);
    ...
    g_signal_connect(G_OBJECT(browse_content_button_), "clicked",
                     G_CALLBACK(EventHandler::click_browse_content_callback), evhandler_);
    ...
    g_signal_connect(G_OBJECT(is_external_domain_), "clicked",
                     G_CALLBACK(EventHandler::click_external_domain_change_callback), evhandler_);
    ...
    connect_signal_id = g_signal_connect(G_OBJECT(connect_button_), "clicked",
                                         G_CALLBACK(EventHandler::click_connect_callback),
                                         evhandler_);
    ...
    g_signal_connect(G_OBJECT(window_), "window-state-event", G_CALLBACK(window_state_event),
                     trayIcon);
}
...
void GtkMainWindow::handleEvent(network::NetworkEvent* event) {
    if (event->eventType() == network::InnerClientConnectedEvent::EventType) {
        ...
        connect_signal_id = g_signal_connect(G_OBJECT(connect_button_), "clicked",
                                             G_CALLBACK(EventHandler::click_disconnect_callback),
                                             evhandler_);
...
void GtkMainWindow::onConnectClicked() {
    ...
    std::string loc_host = gtk_entry_get_text(GTK_ENTRY(domain_text_));
    uint16_t loc_port = common::convertFromString<uint16_t>(gtk_entry_get_text(GTK_ENTRY(port_text_)));
    ...
    old_config.server_type = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(is_external_domain_)) ?
                EXTERNAL_SERVER : FASTO_SERVER;

    std::string exhoststr = gtk_entry_get_text(GTK_ENTRY(external_host_));
    uint16_t ex_port = common::convertFromString<uint16_t>(gtk_entry_get_text(GTK_ENTRY(external_port_)));
