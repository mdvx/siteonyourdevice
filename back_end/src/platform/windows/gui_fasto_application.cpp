...
void Win32MainWindow::showImpl() {
  ...
  const std::string loc_host = cur_config.local_host.host;
  SetWindowText(hwndDomainTextBox_, loc_host.c_str());
  const std::string loc_port = common::convertToString(cur_config.local_host.port);
  ...
  const std::string ex_host = cur_config.external_host.host;
  SetWindowText(hwndExternalHostTextBox_, ex_host.c_str());
  const std::string ex_portstr = common::convertToString(cur_config.external_host.port);
