...
public:
  ...
  static ITcpLoop *
  findExistLoopByPredicate(std::function<bool(ITcpLoop *)> pred);
...
private:
  static void read_write_cb(struct ev_loop *loop, struct ev_io *watcher,
                            int revents);
  static void timer_cb(struct ev_loop *loop, struct ev_timer *timer,
                       int revents);
...
public:
  explicit TcpServer(const common::net::HostAndPort &host,
                     ITcpLoopObserver *observer = nullptr);
  virtual ~TcpServer();
...
private:
  ...
  static void accept_cb(struct ev_loop *loop, struct ev_io *watcher,
                        int revents);
