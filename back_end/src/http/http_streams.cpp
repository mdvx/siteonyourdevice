...
common::http2::frame_t IStream::type() const { return init_frame_.type(); }

IStream::stream_id_t IStream::sid() const { return init_frame_.stream_id(); }

IStream::IStream(const common::net::socket_info &info,
...
common::ErrnoError IStream::sendCloseFrame() {
  common::http2::frame_hdr hdr =
      common::http2::frame_rst::create_frame_header(0, sid());
...
bool HTTP2PriorityStream::processFrameImpl(
    const common::http2::frame_base &frame) {
  sendCloseFrame();
  return true;
}
...
bool HTTP2SettingsStream::isNegotiated() const { return negotiated_; }

bool HTTP2SettingsStream::processFrameImpl(
    const common::http2::frame_base &frame) {
...
bool HTTP2HeadersStream::processFrameImpl(
    const common::http2::frame_base &frame) {
  return false;
}
