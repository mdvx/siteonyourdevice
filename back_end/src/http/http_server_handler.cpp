    ...
    namespace siteonyourdevice
    {
        namespace
        {
           ...
           private:
                ...
                {
                    return new websocket::WebSocketServerHandler(info_);
                }

                tcp::ITcpLoop * createServer(tcp::ITcpLoopObserver * handler)
                {
                    websocket::WebSocketServer* serv = new websocket::WebSocketServer(host_, handler);
        ...
        namespace http
        {
            ...
            bool HttpServerHandler::tryAuthenticateIfNeeded(HttpClient* hclient, const char* extra_header, const common::http::http_request& request)
            {
                ...
                if(authField.isValid()){
                    ...
                    common::Error err = hclient->send_error(common::http::HP_1_1, common::http::HS_UNAUTHORIZED, "WWW-Authenticate: " AUTH_BASIC_METHOD " realm=User or password incorrect, try again", NULL, true, info());
                    if(err && err->isError()){
                        DEBUG_MSG_ERROR(err);
                    }
                    return true;
                ...
                common::Error err = hclient->send_error(common::http::HP_1_1, common::http::HS_UNAUTHORIZED, "WWW-Authenticate: " AUTH_BASIC_METHOD " realm=Private page please authenticate", NULL, true, info());
                if(err && err->isError()){
                    DEBUG_MSG_ERROR(err);
                }
            ...
            void HttpServerHandler::processReceived(HttpClient *hclient, const char* request, size_t req_len)
            {
                ...
                if(result.second && result.second->isError()){
                    ...
                    common::Error err = hclient->send_error(common::http::HP_1_1, result.first, NULL, error_text.c_str(), false, info());
                    if(err && err->isError()){
                        DEBUG_MSG_ERROR(err);
                    }
                    hclient->close();
            ...
            void Http2ServerHandler::handleHttp2Request(Http2Client* h2client, const char* request, uint32_t req_len)
            {
                ...
                for(int i = 0; i < headers_frames.size(); ++i){
                    ...
                    if(result.second && result.second->isError()){
                        const std::string error_text = result.second->description();
                        DEBUG_MSG_ERROR(result.second);
                        common::Error err = h2client->send_error(common::http::HP_2_0, result.first, NULL, error_text.c_str(), false, info());
                        if(err && err->isError()){
                            DEBUG_MSG_ERROR(err);
                        }
                        h2client->close();
