...
public:
  typedef std::function<void(cmd_seq_t request_id, int argc, char *argv[])>
      callback_t;
...
public:
  ...
  cmd_request_t make_request(const char *cmd_fmt, Args... args) {
    ...
    int res = common::SNPrintf(buff, MAX_COMMAND_SIZE, cmd_fmt, REQUEST_COMMAND,
                               id, args...);
    CHECK_NE(res, -1);
...
protected:
  void handleInnerDataReceived(InnerClient *connection, char *buff,
                               uint32_t buff_len);

  template <typename... Args>
  cmd_responce_t make_responce(cmd_seq_t id, const char *cmd_fmt,
                               Args... args) {
    char buff[MAX_COMMAND_SIZE] = {0};
    int res = common::SNPrintf(buff, MAX_COMMAND_SIZE, cmd_fmt,
                               RESPONCE_COMMAND, id, args...);
    CHECK_NE(res, -1);
    return cmd_responce_t(id, buff);
  }

  template <typename... Args>
  cmd_approve_t make_approve_responce(cmd_seq_t id, const char *cmd_fmt,
                                      Args... args) {
    char buff[MAX_COMMAND_SIZE] = {0};
    int res = common::SNPrintf(buff, MAX_COMMAND_SIZE, cmd_fmt, APPROVE_COMMAND,
                               id, args...);
    CHECK_NE(res, -1);
...
private:
  ...
  virtual void handleInnerRequestCommand(
      InnerClient *connection, cmd_seq_t id, int argc,
      char *
          argv[]) = 0; // called when argv not NULL and argc > 0 , only responce
  virtual void handleInnerResponceCommand(InnerClient *connection, cmd_seq_t id,
                                          int argc,
                                          char *argv[]) = 0; // called when argv
                                                             // not NULL and
                                                             // argc > 0, only
                                                             // approve responce
