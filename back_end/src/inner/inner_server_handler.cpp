...
InnerServerHandler::InnerServerHandler(
    const common::net::HostAndPort &innerHost, const HttpConfig &config)
...
void InnerServerHandler::preLooped(tcp::ITcpLoop *server) {
  ping_server_id_timer_ =
      server->createTimer(ping_timeout_server, ping_timeout_server);
  CHECK(!inner_connection_);

  common::net::socket_info client_info;
  common::ErrnoError err = common::net::connect(
      innerHost_, common::net::ST_SOCK_STREAM, 0, &client_info);
  if (err) {
    DEBUG_MSG_ERROR(err, common::logging::LOG_LEVEL_ERR);
    auto ex_event = make_exception_event(
        new network::InnerClientConnectedEvent(this, authInfo()),
        make_error_from_errno(err));
...
void InnerServerHandler::accepted(tcp::TcpClient *client) { UNUSED(client); }

void InnerServerHandler::moved(tcp::TcpClient *client) { UNUSED(client); }

void InnerServerHandler::closed(tcp::TcpClient *client) {
  if (client == inner_connection_) {
    EVENT_BUS()->PostEvent(
        new network::InnerClientDisconnectedEvent(this, authInfo()));
    inner_connection_ = nullptr;
    return;
  }
}
...
void InnerServerHandler::timerEmited(tcp::ITcpLoop *server, timer_id_t id) {
  if (id == ping_server_id_timer_ && inner_connection_) {
    ...
    common::ErrnoError err = client->write(ping_request, &nwrite);
    if (err) {
      DEBUG_MSG_ERROR(err, common::logging::LOG_LEVEL_ERR);
      client->close();
      delete client;
...
void InnerServerHandler::handleInnerRequestCommand(InnerClient *connection,
  ...
  if (IS_EQUAL_COMMAND(command, PING_COMMAND)) {
    ...
    common::ErrnoError err = connection->write(pong, &nwrite);
    if (err) {
      DEBUG_MSG_ERROR(err, common::logging::LOG_LEVEL_ERR);
    }
    ...
    cmd_responce_t iAm =
        make_responce(id, CLIENT_WHO_ARE_YOU_COMMAND_RESP_SUCCSESS_1S, authStr);
    common::ErrnoError err = connection->write(iAm, &nwrite);
    if (err) {
      DEBUG_MSG_ERROR(err, common::logging::LOG_LEVEL_ERR);
    }
    ...
    if (argc > 1) {
      ...
      if (hostandport) {
        ...
        common::ErrnoError err = common::net::connect(
            host, common::net::ST_SOCK_STREAM, nullptr, &rinfo);
        if (err) {
          cmd_responce_t resp =
              make_responce(id, CLIENT_PLEASE_CONNECT_HTTP_COMMAND_RESP_FAIL_1S,
        ...
        tcp::ITcpLoop *server =
            tcp::ITcpLoop::findExistLoopByPredicate(find_by_name);
        if (!server) {
          cmd_responce_t resp =
              make_responce(id, CLIENT_PLEASE_CONNECT_HTTP_COMMAND_RESP_FAIL_1S,
                            CAUSE_CONNECT_FAILED);
        ...
        cmd_responce_t resp = make_responce(
            id, CLIENT_PLEASE_CONNECT_HTTP_COMMAND_RESP_SUCCSESS_1S,
            hostandport);
        err = connection->write(resp, &nwrite);
        if (err) {
          DEBUG_MSG_ERROR(err, common::logging::LOG_LEVEL_ERR);
        ...
        if (config_.server_type == EXTERNAL_SERVER) {
          relayConnection =
              new RelayClientEx(server, rinfo, config_.external_host);
        } else {
  ...
  } else if (IS_EQUAL_COMMAND(command,
                              SERVER_PLEASE_CONNECT_WEBSOCKET_COMMAND)) {
    if (argc > 2) {
      const char *hostandport = argv[1];
      ...
      if (hostandport && hostandport_src) {
        ...
        if (!host_src.IsValid()) {
          cmd_responce_t resp = make_responce(
              id, CLIENT_PLEASE_CONNECT_WEBSOCKET_COMMAND_RESP_FAIL_1S,
              CAUSE_INVALID_ARGS);
          common::ErrnoError err = connection->write(resp, &nwrite);
          if (err) {
            DEBUG_MSG_ERROR(err, common::logging::LOG_LEVEL_ERR);
        ...
        cmd_responce_t resp = make_responce(
            id, CLIENT_PLEASE_CONNECT_WEBSOCKET_COMMAND_RESP_SUCCSESS_1S,
            hostandport);
        err = connection->write(resp, &nwrite);
        if (err) {
          DEBUG_MSG_ERROR(err, common::logging::LOG_LEVEL_ERR);
    ...
    const common::system_info::CpuInfo &c1 =
        common::system_info::CurrentCpuInfo();
    ...
    std::string os =
        common::MemSPrintf("%s %s(%s)", os_name, os_version, os_arch);

    json_object *info_json = json_object_new_object();
    json_object_object_add(info_json, "os", json_object_new_string(os.c_str()));
    json_object_object_add(info_json, "cpu",
                           json_object_new_string(brand.c_str()));
    json_object_object_add(info_json, "ram_total",
                           json_object_new_int64(ram_total));
    json_object_object_add(info_json, "ram_free",
                           json_object_new_int64(ram_free));

    const char *info_json_string = json_object_get_string(info_json);
    cmd_responce_t resp =
        make_responce(id, CLIENT_PLEASE_SYSTEM_INFO_COMMAND_RESP_SUCCSESS_1J,
                      info_json_string);
    common::ErrnoError err = connection->write(resp, &nwrite);
    if (err) {
      DEBUG_MSG_ERROR(err, common::logging::LOG_LEVEL_ERR);
    ...
    const std::string local_host_str =
    ...
    json_object_object_add(
        config_json, CONTENT_PATH_SETTING_LABEL,
    ...
    const std::string external_host_str =
    ...
    for (size_t i = 0; i < config_.handlers_urls.size(); ++i) {
      ...
      json_object_object_add(jhttp_url, "url",
                             json_object_new_string(url.first.c_str()));
    ...
    json_object_object_add(config_json, HANDLERS_URLS_SECTION_LABEL,
                           jhttp_urls);
    ...
    for (size_t i = 0; i < config_.server_sockets_urls.size(); ++i) {
      ...
      json_object_object_add(jsocket_url, "type",
                             json_object_new_string(url.first.c_str()));
      const std::string surl = url.second.GetUrl();
      json_object_object_add(jsocket_url, "path",
                             json_object_new_string(surl.c_str()));
      json_object_array_add(jsockets_urls, jsocket_url);
    }
    json_object_object_add(config_json, SERVER_SOCKETS_SECTION_LABEL,
                           jsockets_urls);

    const char *config_json_string = json_object_get_string(config_json);
    cmd_responce_t resp = make_responce(
        id, CLIENT_PLEASE_CONFIG_COMMAND_RESP_SUCCSESS_1J, config_json_string);
    common::ErrnoError err = connection->write(resp, &nwrite);
    if (err) {
      DEBUG_MSG_ERROR(err, common::logging::LOG_LEVEL_ERR);
    }
    ...
    if (argc > 1) {
      ...
      if (!config_json) {
        cmd_responce_t resp =
            make_responce(id, CLIENT_PLEASE_SET_CONFIG_COMMAND_RESP_FAIL_1S,
                          CAUSE_INVALID_ARGS);
        common::ErrnoError err = connection->write(resp, &nwrite);
        if (err) {
          DEBUG_MSG_ERROR(err, common::logging::LOG_LEVEL_ERR);
      ...
      json_object_object_get_ex(config_json, LOCAL_HOST_SETTING_LABEL,
                                &jlocal_host);
      if (jlocal_host) {
      ...
      json_object_object_get_ex(config_json, EXTERNAL_HOST_SETTING_LABEL,
                                &jexternal_host);
      if (jexternal_host) {
      ...
      json_object_object_get_ex(config_json, CONTENT_PATH_SETTING_LABEL,
                                &jcontent_path);
      if (jcontent_path) {
      ...
      json_object_object_get_ex(config_json, PRIVATE_SITE_SETTING_LABEL,
                                &jprivate_site);
      if (jprivate_site) {
      ...
      json_object_object_get_ex(config_json, SERVER_TYPE_SETTING_LABEL,
                                &jserver_type);
      if (jserver_type) {
        new_config.server_type =
            static_cast<http_server_t>(json_object_get_int(jserver_type));
      }

      json_object *jhandler_urls = NULL;
      json_object_object_get_ex(config_json, HANDLERS_URLS_SECTION_LABEL,
                                &jhandler_urls);
      if (jhandler_urls) {
        ...
        for (int i = 0; i < urls_count; ++i) {
          ...
          if (jurl && jhandler) {
            ...
            new_config.handlers_urls.push_back(
                std::make_pair(url_str, handler_str));
          }
        }
      }

      json_object *jsockets_section = NULL;
      json_object_object_get_ex(config_json, SERVER_SOCKETS_SECTION_LABEL,
                                &jsockets_section);
      if (jsockets_section) {
        int sockets_count = json_object_array_length(jsockets_section);
        for (int i = 0; i < sockets_count; ++i) {
          json_object *jtsocket =
              json_object_array_get_idx(jsockets_section, i);
          ...
          if (jtype && jpath) {
            ...
            new_config.server_sockets_urls.push_back(
                std::make_pair(type_str, common::uri::Url(path_str)));
          }
        }
      }
      ...
      cmd_responce_t resp =
          make_responce(id, CLIENT_PLEASE_SET_CONFIG_COMMAND_RESP_SUCCSESS);
      common::ErrnoError err = connection->write(resp, &nwrite);
      if (err) {
        DEBUG_MSG_ERROR(err, common::logging::LOG_LEVEL_ERR);
      }
  ...
  if (IS_EQUAL_COMMAND(state_command, SUCCESS_COMMAND) && argc > 1) {
    ...
    if (IS_EQUAL_COMMAND(command, PING_COMMAND)) {
      if (argc > 2) {
        ...
        if (!pong) {
          cmd_approve_t resp = make_approve_responce(
              id, PING_COMMAND_APPROVE_FAIL_1S, CAUSE_INVALID_ARGS);
          common::ErrnoError err = connection->write(resp, &nwrite);
          if (err) {
            DEBUG_MSG_ERROR(err, common::logging::LOG_LEVEL_ERR);
          }
        ...
        const cmd_approve_t resp =
            make_approve_responce(id, PING_COMMAND_APPROVE_SUCCESS);
        common::ErrnoError err = connection->write(resp, &nwrite);
        if (err) {
          DEBUG_MSG_ERROR(err, common::logging::LOG_LEVEL_ERR);
          return;
