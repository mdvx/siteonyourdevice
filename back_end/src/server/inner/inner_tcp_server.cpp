    ...
    namespace siteonyourdevice
    {
        namespace server
        {
            namespace inner
            {
                ...
                public:
                    ...
                    virtual void handleMessage(char* channel, size_t channel_len, char* msg, size_t msg_len)
                    {
                        ...
                        *space = 0;
                        ...
                        fasto::siteonyourdevice::inner::RequestCallback::callback_t cb = std::bind(&InnerSubHandler::processSubscribed, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
                ...
                {
                    ssize_t nwrite = 0;
                    const cmd_request_t whoareyou = make_request(SERVER_WHO_ARE_YOU_COMMAND_REQ);
                ...
                void InnerServerHandlerHost::handleInnerRequestCommand(fasto::siteonyourdevice::inner::InnerClient *connection, cmd_seq_type id, int argc, char *argv[])
                {
                    char* command = argv[0];
                ...
                void InnerServerHandlerHost::handleInnerResponceCommand(fasto::siteonyourdevice::inner::InnerClient *connection, cmd_seq_type id, int argc, char *argv[])
                {
                    ssize_t nwrite = 0;
                    ...
                    if(IS_EQUAL_COMMAND(state_command, SUCCESS_COMMAND) && argc > 1){
                        ...
                        if(IS_EQUAL_COMMAND(command, PING_COMMAND)){
                            ...
                            if(argc > 2){
                                ...
                                if(!uauthstr){
                                    const cmd_approve_t resp = make_approve_responce(id, SERVER_WHO_ARE_YOU_COMMAND_APPROVE_FAIL_1S, CAUSE_INVALID_ARGS);
                                    connection->write(resp, nwrite);
                                    goto fail;
                                }
                                ...
                                if(!uauth.isValid()){
                                    const cmd_approve_t resp = make_approve_responce(id, SERVER_WHO_ARE_YOU_COMMAND_APPROVE_FAIL_1S, CAUSE_INVALID_USER);
                                    connection->write(resp, nwrite);
                                    goto fail;
                                }
                                ...
                                if(!isOk){
                                    const cmd_approve_t resp = make_approve_responce(id, SERVER_WHO_ARE_YOU_COMMAND_APPROVE_FAIL_1S, CAUSE_UNREGISTERED_USER);
                                    connection->write(resp, nwrite);
                                    goto fail;
                                }
                                ...
                                const cmd_approve_t resp = make_approve_responce(id, SERVER_WHO_ARE_YOU_COMMAND_APPROVE_SUCCESS);
                                common::Error err = connection->write(resp, nwrite);
                                if(err && err->isError()){
                                    const cmd_approve_t resp2 = make_approve_responce(id, SERVER_WHO_ARE_YOU_COMMAND_APPROVE_FAIL_1S, err->description());
                ...
                void InnerServerHandlerHost::handleInnerApproveCommand(fasto::siteonyourdevice::inner::InnerClient *connection, cmd_seq_type id, int argc, char *argv[])
                {
                    char* command = argv[0];
                ...
                InnerTcpServer::InnerTcpServer(const common::net::hostAndPort& host, tcp::ITcpLoopObserver *observer)
