  ...
  void publish_command_out(const char *msg, size_t msg_len) {
    ...
    if (!res) {
      std::string err_str = common::MemSPrintf(
          "publish_command_out with args: msg = %s, msg_len = " PRIu64
          " failed!",
          msg, msg_len);
      ERROR_LOG() << err_str;
    }
  ...
  virtual void handleMessage(char *channel, size_t channel_len, char *msg,
    ...
    *space = 0;
    ...
    int len = snprintf(buff, sizeof(buff),
                       STRINGIZE(REQUEST_COMMAND) " %s" END_OF_COMMAND,
    ...
    if (*star_seq != ' ') {
      std::string resp =
          common::MemSPrintf("PROBLEM EXTRACTING SEQUENCE: %s", space + 1);
      WARNING_LOG() << resp;
      publish_command_out(resp);
      return;
    }
    ...
    if (!id_ptr) {
      std::string resp =
          common::MemSPrintf("PROBLEM EXTRACTING ID: %s", space + 1);
      ;
      WARNING_LOG() << resp;
      publish_command_out(resp);
      return;
    ...
    InnerTcpServerClient *fclient =
        parent_->parent_->findInnerConnectionByHost(msg);
    if (!fclient) {
      int argc;
      sds *argv = sdssplitargs(cmd, &argc);
      char *command = argv[0];

      std::string resp = common::MemSPrintf(
          SERVER_COMMANDS_OUT_FAIL_2US(CAUSE_NOT_CONNECTED), id, command);
      publish_command_out(resp);
    ...
    size_t nwrite = 0;
    common::ErrnoError err = fclient->TcpClient::write(buff, len, &nwrite);
    if (err) {
      int argc;
      sds *argv = sdssplitargs(cmd, &argc);
      char *command = argv[0];
      ...
      std::string resp = common::MemSPrintf(
          SERVER_COMMANDS_OUT_FAIL_2US(CAUSE_NOT_HANDLED), id, command);
      publish_command_out(resp);
      sdsfreesplitres(argv, argc);
      return;
    }
...
void InnerServerHandlerHost::preLooped(tcp::ITcpLoop *server) {
  ping_client_id_timer_ =
      server->createTimer(ping_timeout_clients, ping_timeout_clients);
}
  ...
  if (ping_client_id_timer_ == id) {
    ...
    for (size_t i = 0; i < online_clients.size(); ++i) {
      ...
      common::ErrnoError err =
          client->write(ping_request.data(), ping_request.size(), &nwrite);
      INFO_LOG() << "Pinged sended " << nwrite << " byte, client["
                 << client->formatedName() << "], from server["
...
void InnerServerHandlerHost::closed(tcp::TcpClient *client) {
  ...
  if (isOk) {
    InnerTcpServerClient *iconnection =
        dynamic_cast<InnerTcpServerClient *>(client);
    if (iconnection) {
      ...
      if (!res) {
        std::string err_str = common::MemSPrintf(
            "publish_clients_state with args: connected_resp = %s failed!",
            connected_resp);
        ERROR_LOG() << err_str;
      }
    }
...
void InnerServerHandlerHost::setStorageConfig(
    const redis_sub_configuration_t &config) {
...
void InnerServerHandlerHost::handleInnerRequestCommand(
    siteonyourdevice::inner::InnerClient *connection, cmd_seq_t id, int argc,
    char *argv[]) {
  char *command = argv[0];
  ...
  if (IS_EQUAL_COMMAND(state_command, SUCCESS_COMMAND) && argc > 1) {
    ...
    if (IS_EQUAL_COMMAND(command, PING_COMMAND)) {
      ...
      if (argc > 2) {
        ...
        if (!uauthstr) {
          cmd_approve_t resp = make_approve_responce(
              id, SERVER_WHO_ARE_YOU_COMMAND_APPROVE_FAIL_1S,
              CAUSE_INVALID_ARGS);
          connection->write(resp, &nwrite);
          goto fail;
        }
        ...
        InnerTcpServerClient *fclient =
            parent_->findInnerConnectionByHost(hoststr);
        if (fclient) {
          cmd_approve_t resp = make_approve_responce(
              id, SERVER_WHO_ARE_YOU_COMMAND_APPROVE_FAIL_1S,
        ...
        cmd_approve_t resp = make_approve_responce(
            id, SERVER_WHO_ARE_YOU_COMMAND_APPROVE_SUCCESS);
        common::ErrnoError err = connection->write(resp, &nwrite);
        if (err) {
        ...
        cmd_approve_t resp = make_approve_responce(
            id, SERVER_WHO_ARE_YOU_COMMAND_APPROVE_FAIL_1S, CAUSE_INVALID_ARGS);
        connection->write(resp, &nwrite);
      }
...
const char *InnerTcpServer::className() const { return "InnerTcpServer"; }

tcp::TcpClient *
InnerTcpServer::createClient(const common::net::socket_info &info) {
