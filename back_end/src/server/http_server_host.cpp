...
namespace {
    int exec_http_server(fasto::siteonyourdevice::http::Http2Server * server) {
    ...
    int exec_inner_server(fasto::siteonyourdevice::server::inner::InnerTcpServer * server) {
        common::Error err = server->bind();
        if (err && err->isError()) {
            DEBUG_MSG_ERROR(err);
            return EXIT_FAILURE;
    ...
    int exec_websocket_server(fasto::siteonyourdevice::server::websocket::WebSocketServerHost * server) {
        common::Error err = server->bind();
        if (err && err->isError()) {
            DEBUG_MSG_ERROR(err);
            return EXIT_FAILURE;
...
void HttpInnerServerHandlerHost::processHttpRequest(http::HttpClient *hclient,
    ...
    if (!innerConnection) {
        ...
        if (refererField.isValid()) {
            ...
            fpath = path.path();
            DEBUG_MSG_FORMAT<512>(common::logging::L_INFO,
                                  "after hpath: %s, fpath %s", hpath, fpath);
        }
