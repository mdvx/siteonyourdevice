...
redisContext * redis_connect(const fasto::siteonyourdevice::server::redis_configuration_t& config) {
...
bool parse_user_json(const std::string& login, const char* userJson,
                     fasto::siteonyourdevice::UserAuthInfo* out_info, hosts_names_t* out_hosts) {
...
void RedisSub::listen() {
    ...
    while (!stop_) {
        ...
        if (redisGetReply(redis_sub, plreply) != REDIS_OK) {
            DEBUG_MSG_FORMAT<128>(common::logging::L_WARNING,
                                  "REDIS PUB/SUB GET REPLY ERROR: %s", redis_sub->errstr);
            break;
        }
