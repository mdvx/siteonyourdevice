...
createPackage() {
    ...
    if [ "$platform" = 'android' ] ; then
        if [ -n "$branding_complex_variables" ]; then
            cmake ../../ -G "Unix Makefiles" -DCMAKE_TOOLCHAIN_FILE=../../cmake/android.toolchain.cmake -DCMAKE_BUILD_TYPE=RELEASE -DOS_ARCH=$os_arch $branding_variables "$branding_complex_variables"
        else
            cmake ../../ -G "Unix Makefiles" -DCMAKE_TOOLCHAIN_FILE=../../cmake/android.toolchain.cmake -DCMAKE_BUILD_TYPE=RELEASE -DOS_ARCH=$os_arch $branding_variables
        fi
        make install -j2
        make apk_release
        make apk_signed
        ...
        if [ -n "$branding_complex_variables" ]; then
            cmake ../../ -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=RELEASE -DOS_ARCH=$os_arch -DCPACK_GENERATOR="$cpack_generator" \
